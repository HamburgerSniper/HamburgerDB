...
import HamburgerDB.execution.*;
import HamburgerDB.optimizer.LogicalPlan;
import HamburgerDB.optimizer.TableStats;
...
import HamburgerDB.transaction.Transaction;
import HamburgerDB.transaction.TransactionId;

public class Parser {
    static boolean explain = false;

    public static Predicate.Op getOp(String s) throws HamburgerDB.ParsingException {
        if (s.equals("="))
    ...
    public LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
            throws IOException, Zql.ParseException, HamburgerDB.ParsingException {
        @SuppressWarnings("unchecked")
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new HamburgerDB.ParsingException("Table "
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            if (si.getAggregate() != null) {
                ...
                if (groupByField != null
                        ...
                        .getColumn()))) {
                    throw new HamburgerDB.ParsingException("Non-aggregate field "
                            + si.getColumn()
        ...
        if (groupByField != null && aggFun == null) {
            throw new HamburgerDB.ParsingException("GROUP BY without aggregation.");
        }
    ...
    public Query handleQueryStatement(ZQuery s, TransactionId tId)
            throws IOException,
            HamburgerDB.ParsingException, Zql.ParseException {
        ...
        if (physicalPlan != null) {
            Class<?> c;
            try {
                c = Class.forName("HamburgerDB.optimizer.OperatorCardinality");
                ...
                c = Class.forName("HamburgerDB.optimizer.QueryPlanVisualizer");
                m = c.getMethod(
    ...
    public Query handleInsertStatement(ZInsert s, TransactionId tId)
        ...
        try {
            ...
            throw new HamburgerDB.ParsingException("Unknown table : "
                    + s.getTable());
        }
            ...
            for (ZExp e : values) {
                ...
                if (zc.getType() == ZConstant.NUMBER) {
                    if (td.getFieldType(i) != Type.INT_TYPE) {
                        throw new HamburgerDB.ParsingException("Value "
                                + zc.getValue()
    ...
    public Query handleDeleteStatement(ZDelete s, TransactionId tid)
            throws
            HamburgerDB.ParsingException, IOException, ParseException {
        int id;
        try {
    ...
    public LogicalPlan generateLogicalPlan(TransactionId tid, String s)
            throws HamburgerDB.ParsingException, IOException {
        ...
        try {
            ...
            if (s instanceof ZTransactStmt)
                ...
                try {
                    ...
                    if (a instanceof HamburgerDB.ParsingException
            ...
            e.printStackTrace();
        } catch (HamburgerDB.ParsingException e) {
            System.out
